...
c = Tkinter.Canvas(window, width=WIDTH, height=HEIGHT, bg='darkred')
c.pack()
...
BUBBLE_CHANCE = 10
# Ліміт часу на початку гри
TIME_LIMIT = 30
# Кількість очок, за яку нараховується бонусний час
BONUS_SCORE = 1000
score = 0
bonus = 0
# Час закінчення гри
end = time.time() + TIME_LIMIT
while time.time() < end: # Головний цикл продовжується, поки не вичерпано час
    if random.randint(1, BUBBLE_CHANCE) == 1:
        create_bubble()
    move_bubbles()
    clean_up_bubbles()
    score += collision()
    # Визначаємо, коли нараховувати бонусний час
    if (int(score / BONUS_SCORE)) > bonus:
        bonus += 1
        end += TIME_LIMIT
    show_score(score) # Виведення кількості очок
    show_time(int(end - time.time())) # Виведення часу, що залишився
    window.update()
    time.sleep(0.01)
